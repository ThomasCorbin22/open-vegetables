...
describe("BlogService testing with blogservice", () => {
  ...
  beforeEach(async () => {
    ...
    blogService = new BlogService(knex);
  });
  ...
  test("blogService should call searchBlogs", () => {
    ...
    return blogService.searchBlogs(query).then((results) => {
      expect(results.length).toBe(1);
      expect(results[0].title).toBe("Some cool post");
      expect(results[0].body).toBe(
        "This is a placeholder blog post. Please select another."
      );
  ...
  test("blogService should call getBlog", () => {
    ...
    return blogService.getBlog(id).then((results) => {
      expect(results.length).toBe(1);
  ...
  test("blogService should call addBlog", () => {
    ...
    return blogService.addBlog(new_post).then((results) => {
      expect(results.length).toBe(1);
  ...
  test("blogService should call updateBlog", () => {
    ...
    return blogService.updateBlog(altered_post, id).then((results) => {
      expect(results.length).toBe(1);
  ...
  test("blogService should call deleteBlog", () => {
    ...
    return blogService.deleteBlog(id).then((result) => {
      expect(result).toBe(true);

      return blogService.listBlogs().then((list) => {
  ...
  test("blogService should call listPictures", () => {
    ...
    return blogService.listPictures(id).then((results) => {
      expect(results.length).toBe(4);
  ...
  test("blogService should call getPicture", () => {
    ...
    return blogService.getPicture(id).then((results) => {
      expect(results.length).toBe(1);
      expect(results[0].picture_URL).toBe(
        "https://images.pexels.com/photos/965117/pexels-photo-965117.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=650&w=940"
      );
      expect(results[0].blog_id).toBe(1);
    });
  });
  ...
  test("blogService should call postPicture", () => {
    ...
    return blogService.addPicture(new_picture).then((results) => {
      expect(results.length).toBe(1);
  ...
  test("blogService should call putPicture", () => {
    ...
    return blogService.updatePicture(altered_picture, id).then((results) => {
      expect(results.length).toBe(1);
  ...
  test("blogService should call deletePicture", () => {
    ...
    return blogService.deletePicture(id).then((result) => {
      expect(result).toBe(true);

      return blogService.getPicture(id).then((list) => {
  ...
  test("blogService should call listCategories", () => {
    ...
    return blogService.listCategories(id).then((results) => {
  ...
  test("blogService should call getCategory", () => {
    ...
    return blogService.getCategory(id).then((results) => {
      expect(results.length).toBe(1);
  ...
  test("blogService should call postCategory", () => {
    ...
    return blogService.addCategory(new_category).then((results) => {
      expect(results.length).toBe(1);
  ...
  test("blogService should call putCategory", () => {
    ...
    return blogService.updateCategory(altered_category, id).then((results) => {
      expect(results.length).toBe(1);
  ...
  test("blogService should call deleteCategory", () => {
    ...
    return blogService.deleteCategory(id).then((result) => {
      expect(result).toBe(true);

      return blogService.getCategory(id).then((list) => {
        expect(list.length).toBe(0);
      });
    });
  });
