...
describe("ReviewService testing with reviewservice", () => {
  ...
  test("reviewService should call listReviews", () => {
    ...
    return reviewService.listReviews(id).then((results) => {
  ...
  test("reviewService should call getReview", () => {
    ...
    return reviewService.getReview(id).then((results) => {
      expect(results.length).toBe(1);
  ...
  test("reviewService should call addReview", () => {
    ...
    return reviewService.addReview(new_review).then((results) => {
      expect(results.length).toBe(1);
  ...
  test("reviewService should call updateReview", () => {
    ...
    return reviewService.updateReview(altered_review, id).then((results) => {
      expect(results.length).toBe(1);
  ...
  test("reviewService should call deleteReview", () => {
    ...
    return reviewService.deleteReview(id).then((result) => {
      expect(result).toBe(true);

      return reviewService.getReview(id).then((list) => {
  ...
  test("reviewService should call listPictures", () => {
    ...
    return reviewService.listPictures(1).then((results) => {
      expect(results.length).toBe(1);
  ...
  test("reviewService should call getPicture", () => {
    ...
    return reviewService.getPicture(id).then((results) => {
      expect(results.length).toBe(1);
      expect(results[0].picture_URL).toBe(
        "https://images.pexels.com/photos/1267320/pexels-photo-1267320.jpeg?auto=compress&cs=tinysrgb&dpr=1&w=500"
      );
  ...
  test("reviewService should call postPicture", () => {
    ...
    return reviewService.addPicture(new_picture).then((results) => {
      expect(results.length).toBe(1);
  ...
  test("reviewService should call putPicture", () => {
    ...
    return reviewService.updatePicture(altered_picture, id).then((results) => {
      expect(results.length).toBe(1);
  ...
  test("reviewService should call deletePicture", () => {
    ...
    return reviewService.deletePicture(id).then((result) => {
      expect(result).toBe(true);

      return reviewService.getPicture(id).then((list) => {
        expect(list.length).toBe(0);
      });
    });
  });
