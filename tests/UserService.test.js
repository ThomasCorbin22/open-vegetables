...
describe("UserService testing with userservice", () => {
  let new_user = {
    ...
    profile_picture_URL:
      "https://scontent-hkg4-1.xx.fbcdn.net/v/t1.0-9/95344172_10156915557110064_2596378485723234304_n.jpg?_nc_cat=102&_nc_sid=09cbfe&_nc_ohc=zHORTMnUlQoAX87U_z5&_nc_ht=scontent-hkg4-1.xx&oh=61a760d68323bb7eeb87778b8eef8b11&oe=5F05DF32",
  };
  ...
  let altered_user = {
    ...
    profile_picture_URL:
      "https://avatars0.githubusercontent.com/u/40209618?s=460&u=2a86e8fa0d42014551e1f81ff24a1720185d66da&v=4",
  };
  ...
  test("userService should call searchUsers", () => {
    ...
    return userService.searchUsers(query).then((results) => {
  ...
  test("userService should call checkSecurity", () => {
    ...
    return userService.checkSecurity(email, answer).then((results) => {
  ...
  test("userService should call lostPassword", () => {
    ...
    return userService.lostPassword(id, answer, password).then((results) => {
      expect(typeof results).toBe("object");
      expect(results[0].id).toBe(1);
      expect(results[0].first_name).toBe("Thomas");
      expect(results[0].password).not.toBe(
        "$2b$10$TeDLJ.CehjeNyXD8ZQnfh.xa6rsFd4x1H.cwdLexoPFuu3r7Q/4ii"
      );
    });
  });
  ...
  test("userService should call updatePassword", () => {
    ...
    return userService
      .updatePassword(id, original_password, new_password)
      .then((results) => {
        expect(typeof results).toBe("object");
        expect(results[0].id).toBe(1);
        expect(results[0].first_name).toBe("Thomas");
  ...
  test("userService should call listUsers", () => {
    ...
    return userService.listUsers().then((results) => {
      expect(results.length).toBe(3);
      expect(results[0].first_name).toBe("Thomas");
      expect(results[0].restaurant_access[0].name).toEqual(
  ...
  test("userService should call getUser", () => {
    ...
    return userService.getUser(id).then((results) => {
      expect(results.length).toBe(1);
  ...
  test("userService should call addUser", () => {
    ...
    return userService.addUser(new_user).then((results) => {
      expect(results.length).toBe(1);
  ...
  test("userService should call updateUser", () => {
    ...
    return userService.updateUser(altered_user, id).then((results) => {
      expect(results.length).toBe(1);
  ...
  test("userService should call deleteUser", () => {
    ...
    return userService.deleteUser(id).then((result) => {
      expect(result).toBe(true);

      return userService.listUsers().then((list) => {
  ...
  test("userService should call listRestaurants", () => {
    ...
    return userService.listRestaurants(id).then((results) => {
      expect(results.length).toBe(1);
  ...
  test("userService should call getRestaurant", () => {
    ...
    return userService.getRestaurant(id).then((results) => {
      expect(results.length).toBe(1);
  ...
  test("userService should call postRestaurant", () => {
    ...
    return userService.addRestaurant(new_restaurant).then((results) => {
      expect(results.length).toBe(1);
  ...
  test("userService should call putRestaurant", () => {
    ...
    return userService
      .updateRestaurant(altered_restaurant, id)
      .then((results) => {
        expect(results.length).toBe(1);
  ...
  test("userService should call deleteRestaurant", () => {
    ...
    return userService.deleteRestaurant(id).then((result) => {
      expect(result).toBe(true);

      return userService.getRestaurant(id).then((list) => {
  ...
  test("userService should call listBlogs", () => {
    ...
    return userService.listBlogs(id).then((results) => {
      expect(results.length).toBe(1);
      expect(results[0].user_id).toBe(1);
  ...
  test("userService should call getBlog", () => {
    ...
    return userService.getBlog(id).then((results) => {
      expect(results.length).toBe(1);
      expect(results[0].user_id).toBe(2);
  ...
  test("userService should call postBlog", () => {
    ...
    return userService.addBlog(new_blog).then((results) => {
      expect(results.length).toBe(1);
  ...
  test("userService should call putBlog", () => {
    ...
    return userService.updateBlog(altered_blog, id).then((results) => {
      expect(results.length).toBe(1);
  ...
  test("userService should call deleteBlog", () => {
    ...
    return userService.deleteBlog(id).then((result) => {
      expect(result).toBe(true);

      return userService.getBlog(id).then((list) => {
        expect(list.length).toBe(0);
      });
    });
  });
  ...
  test("userService should call listRestaurantAccess", () => {
    ...
    return userService.listRestaurantAccess(id).then((results) => {
      expect(results.length).toBe(1);
      expect(results[0].user_id).toBe(1);
      expect(results[0].restaurant_id).toBe(1);
    });
  ...
  test("userService should call getAccess", () => {
    ...
    return userService.getAccess(id).then((results) => {
      expect(results.length).toBe(1);
      expect(results[0].user_id).toBe(2);
      expect(results[0].restaurant_id).toBe(2);
    });
  ...
  test("userService should call postAccess", () => {
    ...
    return userService.addAccess(new_access).then((results) => {
      expect(results.length).toBe(1);
  ...
  test("userService should call putAccess", () => {
    ...
    return userService.updateAccess(altered_access, id).then((results) => {
      expect(results.length).toBe(1);
  ...
  test("userService should call deleteAccess", () => {
    ...
    return userService.deleteAccess(id).then((result) => {
      expect(result).toBe(true);

      return userService.getAccess(id).then((list) => {
        expect(list.length).toBe(0);
      });
    });
  });
